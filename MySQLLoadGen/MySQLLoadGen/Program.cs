    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (replica == "" || replica ==null)
            {
                ...
                mysqlslap m0 = new mysqlslap(process1, master + " --concurrency=" + workload + " --iterations=100  --query=\"select* from employees.dept_emp_latest_date\"  --verbose --create-schema=employees");
                genload(m0);
            }
            else
            {
                ...
                workload = workload / 2;
                mysqlslap m1 = new mysqlslap(process1, master + " --concurrency=" + workload + " --iterations=100  --query=\"select* from employees.dept_emp_latest_date\"  --verbose --create-schema=employees");
                mysqlslap m2 = new mysqlslap(process2, replica + " --concurrency=" + workload + " --iterations=100  --query=\"select* from employees.dept_emp_latest_date\"  --verbose --create-schema=employees");
                genload(m1);
                genload(m2);
            }                        
